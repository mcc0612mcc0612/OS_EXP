               ...
               unsigned char *description)
{
    ...
    */
    unsigned long cmdstart = malloc(cmd_size);
    myPrintf(0x7, "cmd %ld\n", cmdstart);

    strcpy(cmd, ((struct cmd *)cmdstart)->cmd);
    strcpy(description, ((struct cmd *)cmdstart)->description);
    ((struct cmd *)cmdstart)->help_func = help_func;
    ((struct cmd *)cmdstart)->func = func;

    ((struct cmd *)cmdstart)->nextCmd = ourCmds;
    ourCmds = (struct cmd *)cmdstart;

    //myPrintf(0x7, "addNewCmd: new commmand %s added\n\n", cmd);
}
