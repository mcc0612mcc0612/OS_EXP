...
void eFPartitionWalkByAddr(unsigned long efpHandler){
	...
	*/
	eFPartition* eFPartition = efpHandler;
	showeFPartition(eFPartition);
	for(EEB *EEBptr = eFPartition->firstFree;EEBptr != 0; EEBptr = EEBptr->next_start){
		showEEB(EEBptr);
	}
}
...
unsigned long eFPartitionInit(unsigned long start, unsigned long perSize, unsigned long n){
	...
	*/
	eFPartition* eFPartition = start;
	while(perSize % 8 != 0){
		perSize++;
	}
	eFPartition->perSize = perSize;
	eFPartition->totalN = n;
	eFPartition->firstFree = start + eFPartition_size;
	start += eFPartition_size;
	for(int i=0; i<n; i++){
		start += i*(perSize + EEB_size);
		EEB *EEBptr = start;
		if(i == n-1){
			EEBptr -> next_start = 0;
		}
		else{
			EEBptr -> next_start = start + (perSize + EEB_size);
		}
	}
...
unsigned long eFPartitionAlloc(unsigned long EFPHandler){
	...
	*/
	...
	unsigned long firstfree = ((eFPartition*)EFPHandler)->firstFree;
	if(firstfree != 0){
		((eFPartition*)EFPHandler)->firstFree = ((EEB*)firstfree)->next_start;
		return firstfree;
	}
	else{
...
unsigned long eFPartitionFree(unsigned long EFPHandler,unsigned long mbStart){
	...
	*/
	unsigned long firstfree = ((eFPartition*)EFPHandler)->firstFree;
	if(firstfree > mbStart){
		((EEB*)mbStart)->next_start = firstfree;
		((eFPartition*)EFPHandler)->firstFree = mbStart;
	}
	for(; ; firstfree = ((EEB*)firstfree)->next_start){
		if(((EEB*)firstfree)->next_start == 0){
			((EEB*)firstfree)->next_start = mbStart;
			((EEB*)mbStart)->next_start = 0;
		}
		if(((EEB*)firstfree)->next_start > mbStart){
			((EEB*)mbStart)->next_start = ((EEB*)firstfree)->next_start; 
			((EEB*)firstfree)->next_start = mbStart;
			break;
		}
	}
