...
{ // 获取下一个Tsk
     if (currentTsk == NULL)
          return rqFCFS.head;
     if (currentTsk == rqFCFS.tail)
          return rqFCFS.head;
     else
     {
          return currentTsk->nextTCB;
     }
}
     ...
     for (; start;)
     {
          if (start == tsk)
          {
               if (rqFCFS.tail == tsk)
                    if (pre_start == NULL)
                    {
                         rqFCFS.head = start->nextTCB;
                         rqFCFS.tail = pre_start;
                    }
                    else
                    {
                         pre_start->nextTCB = start->nextTCB;
                         rqFCFS.tail = pre_start;
                    }
               else
               {
...
{ // 在进程池中创建一个进程，并把该进程加入到rqFCFS队列中
     ...
     firstFreeTsk->task_entrance = tskBody;
     firstFreeTsk->TSK_State = TSK_WAIT;
     tskStart(firstFreeTsk);
